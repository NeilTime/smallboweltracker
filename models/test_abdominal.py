...
class AbdominalTrackerPlusSegmentationModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        # ───── NEW: supply safe fall-backs when called in “eval” mode ─────
        if 'B' in input:
    ...
    def backward_G(self):
        ...
        fake_squeezed = self.fake_B[self.clgt_available, :].flatten(end_dim=1)
        real_squeezed = self.real_B[self.clgt_available, :].flatten(end_dim=1)
        ...
        if cl_patches_available:
            self.loss_G_A = self.criterionReg(fake_squeezed, real_squeezed) * self.opt.dir_bce_factor - self.opt.dir_bce_offset
    ...
    def compute_visuals(self):
        ...
        with torch.no_grad():
            if self.isTrain:
                ...
                val_results = all_val_results[0, :, :, 0, 0]
                ...
                for gt_shell in range(self.fake_B.shape[1]):
                    ...
                    val_gt_loc = np.where(val_gt > np.amax(val_gt)*0.5)
                    if firstpatch_is_on_cl:
                        firstpatch_gt_cpu = self.real_B[0, gt_shell, :].cpu().numpy()
                        fp_gt_loc = np.where(firstpatch_gt_cpu > np.amax(firstpatch_gt_cpu) * 0.5)

                    valgt_prob = val_gt[val_gt_loc]
                    val_gt_coords = self.get_top2_from_topk(torch.as_tensor(valgt_prob), torch.as_tensor(val_gt_loc))
                    topk_valres_prob, topk_valres_ind = torch.topk(val_result.squeeze(1), 20)
                    val_top2_coords = self.get_top2_from_topk(topk_valres_prob, topk_valres_ind)

                    self.viz_valpatch_gt = self.result_painter(self.viz_valpatch_gt, val_gt_coords, dist=gt_dists[gt_shell])
                    self.viz_valpatch = self.result_painter(self.viz_valpatch, val_top2_coords, dist=gt_dists[gt_shell])
                    ...
                    self.viz_first_patch = self.result_painter(self.viz_first_patch, fp_top2_coords, dist=gt_dists[gt_shell])
                    ...
                    if firstpatch_is_on_cl:
                        ...
                        self.viz_first_patch_gt = self.result_painter(self.viz_first_patch_gt, fp_gt_coords,
                                                                      dist=gt_dists[gt_shell])

        self.opt.phase = prevphase

    def get_top2_from_topk(self, topk_res_prob, topk_res_ind):
        ind = np.squeeze(topk_res_ind.cpu().numpy())
        prob = np.squeeze(topk_res_prob.cpu().numpy())
        res_coords = self.vertices[ind]
        distnorms = []
        for i in range(len(res_coords)):
            distnorms.append(np.linalg.norm(res_coords[0] - res_coords[i]))
        for i in range(len(res_coords)):
            if distnorms[i] < np.mean(distnorms):
                prob[i] = 0
        val_top2_coords = [res_coords[0], res_coords[np.argmax(prob)]]
        return val_top2_coords
    ...
    def compute_top2_meandists(self, pred, goal):
        """ Compute the mean (euclidean) distance between the top 2 (non-close) predictions and any target """
        alldists = []
        #print('debug:')
        #print(f'pred.shape {pred.shape}, goal.shape {goal.shape}')
        for sample in range(pred.shape[0]):
            _, topk_realb_ind = torch.topk(goal[sample, :], 2)
            indrealb = topk_realb_ind.cpu().numpy()
            topk_prob, topk_ind = torch.topk(pred[sample, :], 20)
            top2_predcoords = self.get_top2_from_topk(topk_prob, topk_ind)

            target_coords = self.vertices[indrealb]
            targetdists = []
            for target_coord in target_coords:
                targetdists.append(np.linalg.norm(top2_predcoords - target_coord, axis=1))
