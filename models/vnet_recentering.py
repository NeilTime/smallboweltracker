...
class VNet_recentering(nn.Module):
    def __init__(self, inChans=3, dir_nc=500, seg_nc=2, ngf=64, first_ngf_fac=1, elu=True, nll=False, use_4D=False, side_3D=False, deep_supervision=False, use_dropout=True, sepcoords=False):
        super(VNet_recentering, self).__init__()
        self.first_ngf_fac = first_ngf_fac
        self.deep_supervision = deep_supervision
        self.sepcoords = sepcoords
        ...
        self.down_tr128 = DownTransition(ngf*4, 3, elu, dropout=use_dropout)
        ...
        self.distconv = nn.Conv3d(ngf*8, 1, kernel_size=1, padding=0)
        self.distrelu = nn.ReLU()

        if sepcoords:
            self.dxconv = nn.Conv3d(ngf * 8, 1, kernel_size=1, padding=0)
            self.dyconv = nn.Conv3d(ngf * 8, 1, kernel_size=1, padding=0)
            self.dzconv = nn.Conv3d(ngf * 8, 1, kernel_size=1, padding=0)
    ...
    def forward(self, x):
        ...
        if self.sepcoords:
            ...
            dir4 = self.dirconv2(dir3)
            dist_estimate = self.distrelu(self.distconv(dir3))
            dir_out = self.softmax(dir4)[:, None, :]  # TODO?: support multiple output shells
        ...
        if self.deep_supervision:
            return seg_out, dir_out, dist_estimate, (seg_ds_1, seg_ds_2, seg_ds_3)
        else:
            if self.sepcoords:
                return seg_out, dx, dy, dz
            else:
                return seg_out, dir_out, dist_estimate
